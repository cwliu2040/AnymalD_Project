#!/usr/bin/env python3
"""Reduce one fixed-speed-scale live run to one causal experimental unit."""
...
def valid_requested_hazard(policy: dict, offline: dict) -> tuple[float | None, int, int]:
    valid_by_clock = {
    ...
    unmatched = sum(int(row["policy_clock_ns"]) not in valid_by_clock for row in offline_records)
    eligible = [
...
def build_record(
    run_dir: Path, *, backend: str, profile: str, block: int, arm: str, stage: str,
) -> dict:
    ...
    treatment = _load(run_dir, "speed_scale_trace_validation.json")
    locomotion = summarize_locomotion(trace["samples"])
    hazard_fraction, hazard_count, unmatched = valid_requested_hazard(policy, offline)
    return {
        "schema_version": 1,
        "kind": "slam_speed_scale_causal_run_record",
        ...
        "metrics": {
            **locomotion,
            ...
            "offline_policy_clock_unmatched_count": unmatched,
            ...
            "assigned_command_scale": treatment["assigned_command_scale"],
            "realized_effective_command_scale": treatment["realized_effective_command_scale"],
        },
        "treatment_trace": treatment,
        "gate": {
            "passed": bool(
                treatment["passed"] and offline["gate"]["passed"]
                and map_result["gate"]["passed"] and unmatched == 0
            ),
...
def main() -> int:
    ...
    (run_dir / "speed_scale_run_record.json").write_text(
