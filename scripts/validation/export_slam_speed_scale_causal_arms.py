#!/usr/bin/env python3
"""Export hash-locked fixed-command-scale wrappers around frozen Arm B."""
...
DEFAULT_OUTPUT_ROOT = PROJECT_ROOT / "exported/slam_speed_scale_causal_pilot_v1"
...
class FixedCommandScaleArm(torch.nn.Module):
    """Scale command xyz uniformly; invalid or stale tracking receives zero."""

    def __init__(self, arm_b: torch.nn.Module, scale: float) -> None:
        super().__init__()
        if not 0.0 < scale <= 1.0:
            raise ValueError("assigned scale must be in (0, 1]")
        self.arm_b = arm_b
        self.scale = float(scale)

    def transformed_observation(self, observation: torch.Tensor) -> torch.Tensor:
        transformed = observation.clone()
        tracking = torch.logical_and(
            observation[..., 49:50] >= 0.5,
            observation[..., 50:51] <= 1.0,
        )
        effective_scale = torch.where(
            tracking,
            torch.full_like(observation[..., 49:50], self.scale),
            torch.zeros_like(observation[..., 49:50]),
        )
        transformed[..., 9:12] = observation[..., 9:12] * effective_scale
        return transformed

    def forward(self, observation: torch.Tensor) -> torch.Tensor:
        return self.arm_b(self.transformed_observation(observation))
...
def _comparison(reference: np.ndarray, candidate: np.ndarray) -> dict[str, Any]:
    ...
    return {
        ...
        "allclose_atol_1e-5": bool(np.allclose(reference, candidate, atol=1e-5, rtol=1e-5)),
    }


def _transform_numpy(observations: np.ndarray, scale: float) -> np.ndarray:
    transformed = observations.copy()
    tracking = (observations[:, 49:50] >= 0.5) & (observations[:, 50:51] <= 1.0)
    transformed[:, 9:12] *= np.where(tracking, scale, 0.0).astype(np.float32)
    return transformed


def _export_one(
    *, name: str, scale: float, arm_b: torch.nn.Module,
    base_evaluator: ReferenceEvaluator, observations: torch.Tensor,
    base_metadata: dict[str, Any], base_policy: Path, checkpoint: Path,
    agent_config: Path, output_root: Path,
) -> dict[str, Any]:
    output_dir = output_root / name
    output_dir.mkdir(parents=True, exist_ok=True)
    module = FixedCommandScaleArm(arm_b, scale).eval()
    ...
    values = observations.numpy()
    transformed = _transform_numpy(values, scale)
    expected = base_evaluator.run(None, {"observation": transformed})[0]
    with torch.inference_mode():
        reference = module(observations).numpy()
        jit_output = traced(observations).numpy()
        regenerated_base = arm_b(torch.from_numpy(transformed)).numpy()
    onnx_output = evaluator.run(None, {"observation": values})[0]
    valid = (values[:, 49] >= 0.5) & (values[:, 50] <= 1.0)
    ratios = np.divide(
        transformed[:, 9:12], values[:, 9:12],
        out=np.zeros_like(transformed[:, 9:12]),
        where=np.abs(values[:, 9:12]) > 1e-6,
    )
    checks = {
        "regenerated_arm_B_vs_hash_locked_base": _comparison(expected, regenerated_base),
        "module_vs_hash_locked_base_transformed_observation": _comparison(expected, reference),
        ...
        "valid_command_exact_assigned_scale": bool(
            np.allclose(transformed[valid, 9:12], values[valid, 9:12] * scale, atol=0, rtol=0)
        ),
        "invalid_stale_command_exact_zero": bool(
            np.array_equal(transformed[~valid, 9:12], np.zeros_like(transformed[~valid, 9:12]))
        ),
        "uniform_xyz_scale": bool(
            np.all((np.abs(values[valid, 9:12]) <= 1e-6) | np.isclose(ratios[valid], scale))
        ),
        "finite_actions": bool(np.all(np.isfinite(reference))),
    }
    passed = bool(all(
        value["allclose_atol_1e-5"] if isinstance(value, dict) else value
    ...
    metadata["checkpoint"] = {
        "path": str(checkpoint.relative_to(PROJECT_ROOT)), "sha256": _sha256(checkpoint),
    }
    metadata["artifacts"] = {
    ...
    metadata["policy_extension"] = {
        "architecture": "frozen_arm_B_fixed_uniform_command_scale",
        "dataset_role": "excluded_causal_development",
        "treatment_arm": name,
        "assigned_command_scale": scale,
        "command_offsets": [9, 10, 11],
        "apply_uniformly_to_xyz": True,
        "invalid_or_stale_command": "exact_zero",
        "stale_condition": "normalized_age_gt_1_or_tracking_valid_lt_0_5",
        "base_policy": {
            "path": str(base_policy.relative_to(PROJECT_ROOT)), "sha256": _sha256(base_policy),
        },
        "export_worktree_base_commit": _git_commit(),
        "runtime_ground_truth_inputs": False,
        "runtime_future_label_inputs": False,
        "resolved_agent_config": {"path": "resolved_agent.yaml", "sha256": _sha256(resolved_agent)},
    }
    metadata_path = output_dir / "policy_metadata.yaml"
    metadata_path.write_text(yaml.safe_dump(metadata, sort_keys=False), encoding="utf-8")
    report = {
        "schema_version": 1, "kind": "slam_speed_scale_causal_arm_export",
        "arm": name, "assigned_scale": scale, "passed": passed, "checks": checks,
        "artifacts": {
    ...
    parity_path = output_dir / "export_parity.json"
    parity_path.write_text(json.dumps(report, indent=2) + "\n", encoding="utf-8")
    if not passed:
        raise RuntimeError(f"fixed-scale export failed: {name}")
    return report
...
def main() -> int:
    ...
    base_evaluator = ReferenceEvaluator(onnx.load(str(base_policy)))
    ...
    observations[:, 48] = torch.rand((512,), generator=generator)
    observations[:, 49] = torch.randint(0, 2, (512,), generator=generator).float()
    observations[:, 50] = 1.5 * torch.rand((512,), generator=generator)
    observations[0].zero_()
    observations[0, 48:51] = torch.tensor([1.0, 1.0, 0.0])
    results = [
        _export_one(
            name=name, scale=float(scale), arm_b=arm_b, base_evaluator=base_evaluator,
            observations=observations, base_metadata=base_metadata, base_policy=base_policy,
            checkpoint=checkpoint, agent_config=agent_config, output_root=output_root,
        )
        for name, scale in protocol["treatment"]["arms"].items()
    ]
    manifest = {
        "schema_version": 1, "kind": "slam_speed_scale_causal_arm_exports",
        "protocol": {"path": str(protocol_path.relative_to(PROJECT_ROOT)), "sha256": _sha256(protocol_path)},
        "base_policy": {"path": str(base_policy.relative_to(PROJECT_ROOT)), "sha256": _sha256(base_policy)},
        "passed": all(result["passed"] for result in results),
        "arms": {result["arm"]: result["artifacts"] for result in results},
    }
    output_root.mkdir(parents=True, exist_ok=True)
    (output_root / "export_manifest.json").write_text(json.dumps(manifest, indent=2) + "\n", encoding="utf-8")
