...
- `exp/slam-fastlio2`本機HEAD為
  `36bfd87 授權啟動正式 SLAM confidence 收數`，遠端仍為
  `bc88e75 凍結正式樣本數與穩定性估計量`；本機ahead 1且沒有push授權。
  `bc88e75`的前一個commit為
...
- 使用者已明確授權formal collection與authorization commit；`formal_collection_authorized=true`
  已提交於`36bfd87`。25 blocks／800 live cells已全部完成並正式接受，詳見下方
  「Formal collection results」；這不等於授權push、3,200-cell replay、model48 promotion或
  default switch。800完成後依使用者決策暫停replay，先分析舊C並設計論文用新C。
...
- 使用者目前先比較 LIO-SAM 與 FAST-LIO2，並把標準化的 SLAM
  ...
  完成800-cell formal live matrix後，正式結果顯示外部deterministic supervisor B是強基準，
  舊model48 learned gait C沒有額外價值且相對B在slip／body motion上較差。使用者的主要目標是
  SCI Q1論文而非近期部署，因此後續以B作必要baseline與hard safety boundary，重新設計能證明
  「不只是降速」的localization-risk-aware locomotion C；不得只續訓model48或盲目reward sweep。
...
- 新pilot descriptive C−D支援mechanism、while-stable body-rate下降、tracking survival正方向與
  ...
  body-motion branch，不得宣稱foot-slip reduction。這是excluded pilot方向，已被下節800-cell
  formal結果推翻／削弱，不得混入formal efficacy；promotion仍未授權。
- 原效率rate ratio因11/40個D denominators `<=0.001`且最大ratio約82而不適合作NI。
  ...
  `formal_collection_authorized=true`其後已提交於`36bfd87`；正式model1450與rollback均不變。

### Formal collection results and publication direction (2026-08-21)

- 25 paired blocks／seeds443..467、兩backend×四profiles×A/B/C/D的800/800 accepted live cells
  已完成；formal completeness五項checks全過、800 identities精確等於凍結schedule、100個
  `(paired_block_id, profile)` clusters的10,000次bootstrap有效。Accepted attempts為block443
  attempt3、444..454 attempt1、455 attempt2、456..464 attempt1、465 attempt2、466..467
  attempt1。Block443原attempt／attempt2因並行Isaac/DDS/GPU污染排除；block455 attempt1與
  block465 attempt1各因一格可重現的estimator replay parity failure排除，均完整保留
  `attempt_disposition.json`，未以單格replacement混入正式資料。
- Accepted-only唯讀symlink視圖與正式分析位於ignored
  `outputs/slam_confidence_publication_v1/formal_analysis_v1/`。原分析器曾錯把
  `perception_conditions`中的native也當成formal schedule，與protocol已凍結的
  `formal_conditions: [gradual_support_loss]`矛盾，造成假性`exact_frozen_schedule=false`；
  `analyze_slam_confidence_publication.py`現改讀`formal_conditions`並補回歸測試，3 tests passed。
  此tracked修正與本文件更新尚未commit，任何commit仍需新的使用者明確同意。
- Primary C−D（200 pairs）沒有支持舊C的額外learned gait價值：stance-weighted slip mean差
  `-0.000492 m/s`、95% CI `[-0.006710, 0.005711]`；while-stable roll/pitch-rate mean差
  `+0.002432 rad/s`、CI `[-0.002873, 0.007954]`；tracking RMST差`-0.0442 s`、CI
  `[-0.6937, 0.5375]`；normalized progress差`-0.01506`、CI`[-0.04598, 0.01022]`。
  Efficiency non-inferiority lower bound仍高於`-0.10`，mechanism sidecar證明gait coordinates
  確實啟用，但roll/pitch與survival方向不支持，因此凍結`complete_support=false`。Fall risk差
  `-3.0 pp`、foot-slip-event差`+3.5 pp`、completion差`+0.5 pp`，區間皆跨零。
- Key secondary C−B顯示完整model48方法不如外部supervisor：slip mean差
  `+0.01441 m/s`、CI`[0.00640, 0.02255]`；while-stable body-rate差
  `+0.01824 rad/s`、CI`[0.00844, 0.02875]`；foot-slip-event risk差`+9.0 pp`、CI
  `[3.5, 14.5] pp`。Tracking RMST差`-0.588 s`但CI跨零；normalized progress差
  `-0.00352`且CI跨零；completion差`-3.0 pp`、CI`[-6.0, 0.0] pp`。因此舊C不可promotion，
  也不應只續訓或調reward試圖挽救。
- Decomposition B−A證明外部confidence-conditioned deterministic command supervisor是強而
  必要的baseline：slip差`-0.25809 m/s`、body-rate差`-0.11137 rad/s`、tracking RMST
  `+2.047 s`，三者CI均排除零；fall／base-contact／foot-slip-event risk分別下降
  `10.5/10.0/18.0 pp`。代價是completion下降`18.0 pp`（CI`[-27,-9] pp`），normalized
  progress mean差`-0.02649`但CI跨零，清楚量化安全—完成率trade-off。
- 研究目標已由使用者確認為SCI Q1論文而非近期可用性。後續不是在B／舊C二選一：B保留作
  strongest simple baseline與invalid/stale hard safety boundary；新C必須證明在matched speed
  或matched progress下，仍能降低body motion／slip並延長SLAM survival，排除「只是走更慢」。
  論文方法暫稱localization-risk-aware locomotion adaptation，不預先限定一定是四個gait
  coordinates；可學習完整joint/contact/posture/command behavior，最後再做mechanism analysis。
- 新C設計優先處理舊C四個根因：(1) intent auxiliary本質上接近B；(2) fixed joint-space gait
  bases不足以控制cadence／duty factor／foot clearance／contact timing；(3)單tick confidence／
  valid／age缺少0.5–1 s degradation/recovery歷史；(4)訓練confidence是exogenous schedule，
  body motion不會回饋下一刻SLAM risk。Estimator是重要交互作用（C-v4 GT 0/512、早期estimator
  78/512），但formal四arms共用estimator15，不能單獨解釋C−B差異。
- 下一步先用800-cell accepted artifacts做phase／backend／profile、matched-speed、gait-coordinate
  及GT-vs-estimator診斷，不啟動3,200 replay。新C候選採temporal risk encoder、deployable
  actor／privileged critic或teacher-student、真實FAST/LIO confidence trace與estimator誤差
  randomization；必要時建立body-motion→下一刻confidence／tracking hazard surrogate形成閉環。
  新候選先過多seed simulation及confidence constant／shuffle／delay因果ablations，再只跑
  B/C的2 backends×4 profiles×2 arms×3 blocks＝48-cell live qualification；沒有matched-speed
  優勢即淘汰。下一份正式protocol須預先加入futility stopping，不再直接投入800 cells。

#### 800-cell stratified diagnosis (2026-08-21)

- 已新增獨立exploratory分析器`diagnose_slam_confidence_publication.py`，只讀accepted run
  records、locomotion diagnostics與C-arm mechanism sidecars，不讀rosbag、不跑SLAM replay，
  以`(paired_block_id, profile)`做10,000次cluster bootstrap。完整解讀在
  `docs/validation/slam_confidence_publication_stratified_diagnostics.md`，ignored機讀結果在
  `outputs/slam_confidence_publication_v1/formal_analysis_v1/stratified_diagnostics.json`。
- C-D沒有在matched speed下恢復價值：slip`-0.00114 m/s`、body-rate`+0.00212 rad/s`、RMST
  `-0.071 s`，區間全跨零。Ramp-down 3--9 s反而slip/body-rate/speed分別增加
  `0.00704 m/s`、`0.00742 rad/s`、`0.02194 m/s`且區間排除零；low-support與recovery沒有
  C-D收益。舊C的learned gait問題集中在退化起始反應，不是恢復期帶回收益。
- C-B的raw harm與速度糾纏：equal-progress regression仍保留slip`+0.01442 m/s`與body-rate
  `+0.01826 rad/s`傷害；equal-speed regression／threshold matching則在slip與body-rate間
  給出method-sensitive的反轉，但沒有可靠RMST或progress收益。因此不能宣稱舊C在matched
  speed更好；應在新C實驗中直接randomize matched command/speed，而非事後調整mediator。
- Backend heterogeneity只在C-D slip interaction有區間排除零：FAST為`-0.00597 m/s`、LIO為
  `+0.00499 m/s`，paired-effects差`-0.01096 m/s`。C-B harm在LIO較強。Profile上右轉curve的
  C-D slip/body-rate惡化；warehouse雖有progress小幅增加，沒有body-motion或survival收益。
- C-arm gait coordinate和outcome的多數raw correlation在控制moving speed後消失。仍存在的
  exploratory partial associations包括stance width對slip/body-rate為正、crouch對body-rate為負、
  action smoothing對body-rate為正、stride attenuation對RMST為負；這些是joint actor下的
  ecological association，不是coordinate ablation，足以反對盲調同一4-D head但不能作因果主張。
- Formal challenge nominal healthy 0--3 s早於route command 5 s warmup，因此沒有active-speed
  healthy estimand；perception phase與command phase部分alias，且run record沒有精確adapter phase
  origin。後續protocol必須把active healthy control與degradation onset對齊並記錄phase origin。
- Formal四arms共用estimator15，GT-vs-estimator effect無法由800格識別；既有C-v4 0/512對
  78/512與final model48+estimator15 6/2560只支持interaction值得納入randomization，不能解釋
  formal C-B/C-D差異。診斷仍不支持model48 promotion、default switch或啟動3,200 replay。

#### Existing-800 temporal risk predictability audit (2026-08-21)

- 使用者確認後只做既有accepted-artifact離線audit；未讀raw bag、未跑ROS／Isaac／PPO、未收
  新資料，也未刪除任何舊validation或log。A的policy diagnostics只有48-D、缺SLAM state，故
  固定排除；B/C/D 600 runs中596有完整causal prediction window，共108,866個10 Hz ticks。
  Frame可訓練predictor，但比較以完整`(paired_block_id, profile)`cluster做10,000次bootstrap。
- 固定比較1 s的SLAM-state-only、motion-without-estimator-velocity、motion-with-estimator-velocity
  histories；模型為training-only標準化、equal-run weighting、L2 logistic regression。Endpoints
  是0.5 s tracking hazard與invalid後1.0 s offline-usability recovery；block 5-fold與
  leave-one-arm-out均分FAST/LIO評估。完整方法與數值在
  `docs/validation/slam_risk_predictability_audit.md`；ignored機讀產物在
  `outputs/slam_confidence_publication_v1/formal_analysis_v1/risk_predictability_audit_v1/`。
- 事前joint gate結論為`FAIL`，必須停止原定「hazard＋recovery一起直接做新C」路線。Hazard
  仍有明確development signal：FAST block/arm holdout Brier分別改善`-0.01273/-0.02560`，LIO
  arm holdout改善`-0.08866`；但LIO block calibration未過。Recovery不穩，FAST block Brier
  明確惡化`+0.02456`、CI`[+0.00545,+0.04395]`，其餘多格ECE亦惡化超過0.02。
- 加入estimator15 linear velocity幾乎不改結論，不能以此證明新模型需要或應依賴estimator
  velocity。保留既有confidence與B deterministic invalid/stale/recovery safety；若重設方法，
  只能把motion-conditioned 0.5 s hazard當development finding，先另凍結尤其針對LIO block
  calibration的新gate，再決定是否做小型policy pilot。現有800不能再作未來新C blind test。
- 此FAIL不是把confidence工程重來，也不是刪除舊資料的理由；相反地，既有artifact是提前停止
  錯誤長訓練／live campaign的證據。不得promotion model48、切default、啟動3,200 replay、
  commit或push。
